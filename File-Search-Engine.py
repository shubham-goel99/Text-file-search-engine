...
def getfolderaddress():
	global address
	address = raw_input("Plese enter folder address(if want to quit press -1)  \n")
	while True: 
		if address=="-1" :
			exit()
			...
			address = raw_input("Enter correct address(if want to quit press -1) \n")
...
# 		lineno=1
# 		pleasedoit(file,lineno)

# def pleasedoit(file,lineno):
# 	global vect5_for_docs 
...
while True:
    # print time.time()
    query = raw_input("Please enter your query....(to quit just press enter)")
    if len(query) == 0:
        break
    ...
    while True:
        query2 = raw_input("To see line of that doc write doc name.For another query write '-1' \n")
        if query2=="-1":
        	break
        check=address +'/' + query2
        if os.path.isfile(check):
            x = set()
            F = open(address +'/' + str(query2)).read().splitlines()
            lineno=0
            vect5_for_docs = defaultdict(list)
            for line in F:
                lineno=lineno+1
                r=nltk.word_tokenize(line)
                for word in r:
                    vect5_for_docs[word].append(lineno)
                for word in query_list:
            		f=vect5_for_docs[word]
            		for r in f:
            			x.add(r)
            for n in x:
        		try:
        			print str(n) + ': ' + F[int(n) - 1]
        		except ValueError:
        			pass
        else:
        	print("File name does not exist. Please enter again ")
